...
pub fn execute_update_node(opts: UpdateNodeOpts) -> Result<(), Error> {
    ...
    if find_micro_service(&chain_config, NETWORK_P2P) {
        ...
        // load cert
        let ca_cert = format!("{}/{}", CA_CERT_DIR, CERT_PEM);
        let cert = format!("{}/{}/{}", CERTS_DIR, &opts.domain, CERT_PEM);
        let key = format!("{}/{}/{}", CERTS_DIR, &opts.domain, KEY_PEM);
        ...
        let network_config = ZenohConfig::new(
            node_config.network_listen_port,
            node_config.grpc_ports.network_port,
            ca_cert,
            cert,
            ...
            validator_address,
            chain_config.system_config.chain_id.clone(),
        );
        network_config.write(&config_file_name);
        // only for new node
        if !is_old {
